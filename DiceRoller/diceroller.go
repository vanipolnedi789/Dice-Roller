...
// call to respective roll function(http api source/internal source) is based the randomGenerator type we are passing
// adds all roll values and writes the total into ResultChannel
func sumRollValue(rolls []string, ResultChannel chan int) {
	total := 0
	var randomGenerator internalRandNumGeneratorSource
	for _, rollValue := range rolls {
		if strings.Contains(rollValue, "d") {
			...
			total += rollIndividualDice(randomGenerator, rollsCount, diceSize)
		} else {
